...
#Construct a Tree
class Tree:
    def __init__(self, board, up=None, left=None, down=None, right=None, parent=None, depth=0):
        self.board = board
        self.up = up
        self.left = left
        self.down = down
...
#Breath-first:
#For each element in the row: Build the next row's state.
#Arbitrarily order up/right/left/down for order of visitation/adding.
#For each node: Check to make sure it's not the goal state.
# Attempt to branch out up/down/left/right. If not the parent and not in the "seen" list:
    #Build out a new board. Add it to a tree leaf, setting parent, and path from parent to node.
...
result = solsearch(depth,maxdepth,openlist,closelist,grid_goal)
aftime = time.time()
