    ...
    for x in board:
        for y in x:
            ...
            if y != 0:
                ...
                if board[posx][posy] != goal[posx][posy]:
                    manhdist += 1
    ...
    while openlist:
        ...
        if left and new_left.depth < maximum:
            ...
            if new_left.board not in seenboards:
                new_up.manh = calcmanh(new_up.board,goalstate)
                #Newly explored node, just link it to x
                x.left = new_left
        ...
        if right and new_right.depth < maximum:
            new_right.manh = calcmanh(new_right.board,goalstate) + new_right.depth
